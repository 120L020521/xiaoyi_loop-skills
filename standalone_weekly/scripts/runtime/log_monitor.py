...
def terminal_stop_reason(text: str) -> str | None:
    """Return the latest main-Agent terminal stop reason that ends execution."""
    ...
    for line in text.splitlines():
        ...
        if isinstance(assistant, dict):
            reason = assistant.get("stop_reason")
            if reason in {"stop", "error"}:
                terminal = reason
    return terminal


def has_stop_reason_stop(text: str) -> bool:
    """Return whether the latest main-Agent terminal reason is ``stop``."""
    return terminal_stop_reason(text) == "stop"
...
def read_remote_stop_candidates(
    ...
    *,
    ...
    stop_pattern = shell_quote(
        r'"stop_reason"[[:space:]]*:[[:space:]]*"(stop|error)"'
    )
    command = (
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        if active_log is not None:
            ...
            if reason == "error":
                raise TaskExecutionError(
                    f"{item_id} main agent ended with stop_reason=error.",
        ...
        if now - last_wait_print >= 300:
            print(f"[{item_id}] Waiting for main agent stop_reason=stop/error ...")
            last_wait_print = now
