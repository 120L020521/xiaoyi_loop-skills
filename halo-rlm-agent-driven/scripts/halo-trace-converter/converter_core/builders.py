...
from .status import llm_status, tool_status
...
def span(
    *,
    ...
    return {
        ...
        # The source event log does not provide an OTel service identity.
        # Keep the resource empty instead of presenting the converter's
        # instrumentation scope as the service that executed the task.
        "resource": {"attributes": {}},
        "scope": {"name": "pi-agent", "version": "1.0.0"},
        "attributes": attrs,
    }
...
def _native_output_message(assistant: Json) -> Json:
    """Match pi-halo-tracer's normalized assistant output message shape."""
    text_parts: list[str] = []
    ...
    if isinstance(content, list):
        for item in content:
            ...
            if isinstance(item.get("text"), str):
                ...
                continue
            elif item.get("id") not in (None, "") and item.get("name") not in (
                None,
                "",
            ):
                tool_calls.append(
...
def llm_span(
    ...
    attrs = {
        **base_attrs(project_id, "LLM"),
        ...
        "llm.output_messages": attribute_value(
            [_native_output_message(assistant)] if assistant else payload,
            options,
        ),
    ...
    error_message, failed = llm_status(payload)
    start_time = (
...
def tool_span(
    ...
    result_payload = {
        ...
        if key not in {"is_error", "tool_call_id", "tool_name"}
    }
    attrs = {
        **base_attrs(project_id, "TOOL"),
        ...
        "output.value": attribute_value(result_payload, options),
        "tool.is_error": failed,
        "source.tool_result.context": source_context_value(
            row,
            {key: "output.value" for key in result_payload},
            options,
        ),
    }
    if call:
