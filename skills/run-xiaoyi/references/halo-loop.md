# Embedded Judge-to-HALO batch orchestration

Load this reference only after `run-xiaoyi` selects `RUNNER_JUDGE_DIAGNOSE` or
receives an existing batch `handoff.json`. This workflow coordinates phases; it
does not perform actual Trace diagnosis itself.
...
- Resolve sibling skills from the same installed Skill collection:
  `run-xiaoyi-loop`, `judge-xiaoyi-results`, and `halo-rlm-agent-driven`.
- Read all three sibling `SKILL.md` files completely before acting and follow
  their phase-specific rules.
- Resolve `<run_xiaoyi_root>/scripts/halo-loop` as `<halo_loop_scripts>` and use
  `<halo_loop_scripts>/handoff.py` as the only handoff parser and validator.
...
```powershell
& <python> "<halo_loop_scripts>\handoff.py" create `
...
```powershell
& <python> "<halo_loop_scripts>\handoff.py" resolve `
...
Give each subagent only the `halo-rlm-agent-driven` Skill root, Task ID,
...
```powershell
& <python> "<halo_loop_scripts>\handoff.py" summarize `
  "<resolved_halo_output>\handoff.json"
```
