# Embedded HALO diagnosis contract

Load this reference only after `run-xiaoyi` selects an explicit HALO diagnosis
stage or private `halo-diagnose-one` mode. Resolve
`<run_xiaoyi_root>/scripts/halo` as `<halo_scripts>` and use that directory as
the working directory for every `python -m halo_rlm...` command below.
...
```bash
python "<halo_scripts>/prepare_trace.py" INPUT --output-root OUTPUT_ROOT
```
...
Then run exactly once for each prepared trace from `<halo_scripts>`:
...
Read `references/halo-trace-format.md` only for span shape/truncation and
`references/halo-architecture.md` only for recursion, compaction, or termination.
