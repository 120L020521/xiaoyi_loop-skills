...
def convert_directory_files(
    ...
    *,
    ...
    """Convert every JSONL independently and preserve one output per source.

    Cross-file subagent linking is intentionally not performed. A
    ``run_subagent``/``call_subagent`` event in a main log remains a normal
    TOOL span, while a separately recorded child log becomes its own trace with
    an AGENT root. This keeps file boundaries and source evidence lossless.
    """
    ...
    for source in files:
        ...
        print(
            ...
            f"spans={len(spans)} skipped={skipped}"
        )

    return converted, skipped_total, skipped_files
