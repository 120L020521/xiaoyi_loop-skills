...
description: >-
  ...
  Runner timeouts and failures, through run-xiaoyi-loop, judge-xiaoyi-results,
  and the HALO diagnosis capability embedded in run-xiaoyi. Use when users ask
  to run and Judge multiple XiaoYi tasks and diagnose all or only failed
  current-batch tasks with HALO, or when they provide a batch handoff.json for
  HALO diagnosis. This is an internal batch coordinator selected by run-xiaoyi.
---

# Run XiaoYi + Judge + HALO

Act only as a thin batch coordinator. Do not copy or reinterpret Runner, Judge,
trace-inspection, or report-generation logic from the owning skills.

## Required components

- Use Python >= 3.10 and UTF-8.
- Resolve `run-xiaoyi-loop`, `judge-xiaoyi-results`, and `run-xiaoyi` from the
  same installed Skill collection.
- Read the first two `SKILL.md` files completely. For diagnosis, read
  `run-xiaoyi` completely and use only its private `halo-diagnose-one` mode plus
  `references/halo-diagnosis.md`; do not re-enter its top-level batch routing.
...
Judge result, and output root to a fresh worker applying `run-xiaoyi` in
`halo-diagnose-one` mode. Omit `--surface` so embedded HALO uses its logical
default target-name allowlist; no surface files or surface directory are
handoff inputs.
...
This coordinator currently accepts numeric WorkspaceBench Task IDs because
`handoff.py` schema v3 and its fixed `task<ID>` paths are numeric. The shared
Judge may evaluate FileOrganization cases independently, but do not send those
string case IDs into HALO until a future handoff schema explicitly supports
them.
...
### 1. Run and prepare current Task evidence

- For Task selectors or Task directories, apply `run-xiaoyi-loop` in its explicit
  `runner-and-prepare-only` mode. Wait for its Runner and prepare phases to
  finish. Never launch a duplicate Runner. The current batch is the
  deduplicated union of `runner.completed` and `runner.failed` Task IDs. All use
...
### 2. Judge, then create one batch handoff

Apply `judge-xiaoyi-results` with its `workspacebench` adapter to every prepared
current-batch Task. Wait until every Task has a verified Judge result or a
recorded Judge/prepare/Runner failure. Never copy Agent Judge rules into this
coordinator and never let `run-xiaoyi-loop` run a second Judge phase.
...
Give each subagent only the `run-xiaoyi` Skill root, Task ID,
`paths.trace_jsonl`, `roots.halo_output`, and that Task's
`build_prompt_inputs`. Require it to:

1. Apply `run-xiaoyi` in private `halo-diagnose-one` mode only to its assigned
   raw trace. Never re-enter top-level routing, use
   `normalized_runner_log.jsonl`, or inspect another Task.
...
4. Apply the embedded HALO contract through completion, then return its manifest
   and report paths. Do not copy, invoke, or reinterpret its internal
   report-validation procedure in this coordinator.
...
HALO worker use the embedded contract's bounded single-agent fallback. The
parent must not run `validate-report`; report acceptance belongs entirely to
the embedded diagnosis workflow. Let the
